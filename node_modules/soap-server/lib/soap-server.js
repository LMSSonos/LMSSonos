...
function SoapOperation(name, service, operation){
	...
	//console.log("debug="+operationString);
	
	var args = null;
	
	//fix for methods without params
	if (matches != null)
	{
...
SoapServer.prototype.handleOperationRequest = function(service, soapRequest){
	var operationRequest = soapRequest['s:Envelope']['s:Body'][0];
...
SoapServer.prototype.js2xml = function(js, namespace, indentLevel, responseName){
	var ret = '';
	var indent = '';
	...
	for(var node in js){
		...
		if('object' == typeof js[node]){
			if (element == "mediaCollection"
				|| element == "mediaMetadata") //pb: special rendering
			{
				js[node].forEach(function(child) {
					ret += indent + '<' + element + '>\n';
					ret +=this.js2xml(child, namespace, indentLevel + 1, responseName);
					ret += indent + '</' + element + '>\n';
					//	ret += indent + element;
			...
			{
				//rename return with the nice naming
				if (element == "return")
				{
					element =  responseName + "Result";
				}
				
				if (element.indexOf('Response') == (element.length - 8))
				{
					ret += indent + '<' + element + ' xmlns="http://www.sonos.com/Services/1.1">\n';
					responseName = element.substr(0,element.length-8);
				}
				else
					ret += indent + '<' + element + '>\n';
					
				ret += this.js2xml(js[node], namespace, indentLevel + 1, responseName);
				ret += indent + '</' + element + '>\n';
			}	
...
SoapServer.prototype.handleRequest = function(req, res){
	console.log("handleReqest started");
	try{
...
SoapServer.prototype.handleSoapRequest = function(req, callback){
	...
	if (req.url == "/presentationmap.xml")
	{
		//return presentation map xml
		var responseXml = fs.readFileSync("presentationmap.xml").toString().trim();;
		callback.apply(null, [responseXml]);
		console.log("presentationamp.xml returned");
		return;
	}
		...
		req.on('end', function () {
			...
			parseXml(body, function (err, soapRequest) {
				...
				try{
					...
					//console.dir('a1='+soapResponse);
					//console.log('a2='+soapRequest.operationName);				
